...
namespace CachingFramework.Redis.RedisObjects
{
    ...
    internal class RedisDictionary<TK, TV> : RedisBaseObject, IRedisDictionary<TK, TV>, IDictionary<TK, TV>
    {
        ...
        public async Task AddAsync(TK key, TV value, string[] tags)
        {
            if (tags == null || tags.Length == 0)
            {
                await AddAsync(key, value).ForAwait();
                return;
            }
            await _cacheProvider.SetHashedAsync<TK, TV>(RedisKey, key, value, tags).ForAwait();
        }
        ...
        {
            await GetRedisDb()
                .HashSetAsync(RedisKey, items.Select(i => new HashEntry(Serialize(i.Key), Serialize(i.Value))).ToArray()).ForAwait();
        }
        ...
        {
            await _cacheProvider.SetHashedAsync(RedisKey, items, tags: tags).ForAwait();
        }
        ...
        {
            return await GetRedisDb().HashLengthAsync(RedisKey).ForAwait();
        }
        ...
        {
            await GetRedisDb().HashSetAsync(RedisKey, Serialize(key), Serialize(value)).ForAwait();
        }
        ...
        {
            return await GetRedisDb().HashExistsAsync(RedisKey, Serialize(key)).ForAwait();
        }
        /// <summary>
        ...
        {
            return await GetRedisDb().HashExistsAsync(RedisKey, Serialize(item.Key)).ForAwait();
        }
        /// <summary>
        ...
        {
            return await GetRedisDb().HashDeleteAsync(RedisKey, Serialize(key)).ForAwait();
        }
        ...
        {
            var redisValue = await GetRedisDb().HashGetAsync(RedisKey, Serialize(key)).ForAwait();
            return Deserialize<TV>(redisValue);
        }
        /// <inheritdoc />
        ...
        {
            return await _cacheProvider.GetHashedAsync<TK, TV>(RedisKey, keys).ForAwait();
        }
        #endregion
        ...
        {
            var db = GetRedisDb();
            return await db.HashIncrementAsync(RedisKey, Serialize(key), increment).ForAwait();
        }
        /// <inheritdoc />
